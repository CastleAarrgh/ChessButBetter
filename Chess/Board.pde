...
class Board {
 ...
 public Board() {
  ...
  public void displayBoard() {
    final int Xwidth = 100;
    final int Ywidth = 100;
    ...
    for (int i = 0; i < 8; i ++) {
      for (int j = 0; j < 8; j ++) {
        ...
        rect(i * Xwidth, j * Ywidth, (i + 1) * Xwidth, (j + 1) * Ywidth);  
        if (board[j][i] != null) image(board[j][i].getPieceImage(), i*100, j*100, squareSize, squareSize);
      }
    }
    if (promote && !gameOver) {
      stroke(2);
...
{
  return board[x][0].getXcordinate();
}

int getYcordinate(int y)
{
  return board[0][y].getYcordinate();
}
